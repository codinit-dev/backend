...
router.get('/', async (req, res) => {
  try {
    ...
    const isAdmin = user.role === 'admin' || (userTeam?.teams)?.tier === 'enterprise';
    ...
    return res.status(500).json({ error: 'Failed to fetch business analytics' });
  }
});

// Revenue Metrics (High-value monetization data)
async function getRevenueMetrics(startDate, isAdmin) {
  const supabase = createServerClient();
...
// User Analytics
async function getUserMetrics(startDate, isAdmin) {
  const supabase = createServerClient();
...
// Product Analytics
async function getProductMetrics(startDate, isAdmin) {
  const supabase = createServerClient();
...
// Churn Analysis
async function getChurnAnalysis(startDate, isAdmin) {
  const supabase = createServerClient();
...
// Trend Data
async function getTrendData(timeRange, isAdmin) {
